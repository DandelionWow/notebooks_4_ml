...
其中，$α$表示决定两个预训练模型之间权重的超参数（<u>在代码中好像只做了连接操作，没有加入这个超参数</u>）。

对于训练GMF和MLP，采用了自适应矩估计，它通过对频繁的参数执行较小的更新和对不频繁的参数执行较大的更新来适应每个参数的学习率。在将预训练的参数输入NeuMF后，我们使用朴素SGD而不是Adam来优化它。（<u>但是，在代码中并没有见到使用SGD</u>）